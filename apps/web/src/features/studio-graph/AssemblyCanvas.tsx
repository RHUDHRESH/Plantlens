  ...
  const edges: Edge[] = useMemo(
    () =>
      assembly.connections.map((conn) => ({
        ...
        labelStyle: {
          fill: conn.approved ? "var(--accent)" : "var(--ink-500)",
          fontSize: 10,
          fontWeight: 600,
          letterSpacing: "0.05em",
        },
        labelBgStyle: {
          fill: conn.approved ? "var(--accent-tint)" : "var(--surface)",
          stroke: conn.approved ? "var(--accent)" : "var(--line)",
          strokeWidth: 1,
        },
        selected: conn.connection_id === selectedConnectionId,
        className: conn.approved ? "edge-approved" : "edge-draft",
        style: {
          stroke: conn.approved ? "var(--accent)" : "var(--ink-300)",
